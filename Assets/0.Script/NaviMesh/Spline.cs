...
public class Spline : MonoBehaviour
{
        ...
        Vector3 endPos, Vector3 midPos)
    {
        ...
        Vector3 middleEnd = Vector3.Lerp(endPos, midPos,value01);
        return Vector3.Lerp(startMiddle, middleEnd, value01);
    }

    public Vector3 CalculatePosition(float interpolationAmout01)
        => CalculatePosition(interpolationAmout01,
            _start.position, _end.position, _middle.position);
    
    public Vector3 CalculatePositionCstomStart(float interpolationAmout01,
        Vector3 startPosition)
    => CalculatePosition(interpolationAmout01,startPosition, _end.position, _middle.position);
